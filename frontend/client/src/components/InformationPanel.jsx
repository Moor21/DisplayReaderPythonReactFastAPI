  ...
  const sendHandler = async () => {
    if (minThresh < maxThresh) {
      const parameters = {
        minThresh: minThresh,
        maxThresh: maxThresh,
        blur_kernel: blurValue,
        morph_kernel: morphValue,
        xFactor: xFactor,
        yFactor: yFactor,
      };
      const data = await fetch("http://127.0.0.1:8000/parameters/send", {
        method: "POST",
        headers: {
      ...
      alert("MinThresh should be smaller than maxThresh!");
    }
  };
      ...
      <div className="display-status-box">
        ...
        <div className="rows">
          ...
          <div className="display-status-subbox">
            ...
            <span className="status-main-text">
              {numbersFromDisplay ? numbersFromDisplay : "None"}
            </span>
            <span className="status-footer-text">
              {!numbersFromDisplay && "No digits detected"}
            </span>
          </div>
        </div>
      </div>
      <div className="conf-menu">
        ...
        <div className="form">
          <div className="columns">
            <label className="main_thresh_label" htmlFor="">
              Threshold
            </label>
            <label className="main_thresh_laber" htmlFor="">
              Blur kernel size
            </label>
            <label className="main_thresh_laber" htmlFor="">
              Morph kernel size
          ...
          <div className="columns">
            <label className="small_thresh_label" htmlFor="">
              {" "}
              Min{" "}
              <input
                type="range"
          ...
          <div className="columns">
            ...
            <input
              className="num_inp"
              value={yFactor}
              onChange={(e) => setYFactor(Number(e.target.value))}
              min={-1}
              max={2}
              step={0.1}
              placeholder="fy..."
          ...
          <button onClick={sendHandler} className="btn">
            Send
          </button>
        </div>
      </div>
